...
parser.add_argument('--tier',           default = "datasets/gqa/testdev_balanced",                     type = str,    help = "Tier, e.g. train, val")
...
predictions = {p["question_id"]: p["answer"] for p in predictions}

# Make sure all question have predictions
for qid in questions:
    if (qid['question_id'] not in predictions) and (args.consistency or qid["isBalanced"]):
...
if args.grounding:
    with open(args.attentions.format(tier = args.tier)) as attentionsFile:
        ...
        attentions = {a["question_id"]: a["answer"] for a in attentions}
