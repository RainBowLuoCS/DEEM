...
def load_model_weights(model, ckpt_path, image_upscale=1.0):
    ...
    pos_embed_key = (
        "visual_tokenizer.sniffer.vision_model.embeddings.position_embedding.weight"
    )
    is_vit_l = pos_embed_key in pretrained_weights.keys()
    if 'visual_tokenizer.sniffer.vision_model.embeddings' in pretrained_weights.keys():
        if not is_vit_l:
            pos_embed_key = "visual_tokenizer.sniffer.pos_embed"
