...
def load_laion_database_nothrow(
    ...
    for sample in src:
        ...
        try:
            ...
            #     lines = rf.readlines()
            lines=wds.tariterators.tarfile_samples([{'url': data_path}], handler=handler)
            lines=wds.filters._decode(lines,"pilrgb", handler=handler)
            lines=wds.filters._to_tuple(lines, "jpg", "json", handler= handler)
...
def preprocess_laion_data(
    ...
    info, meta_info = sample[0], sample[-1]
    ...
    caption = _smart_join(info[1]["caption"], " ").lower()
