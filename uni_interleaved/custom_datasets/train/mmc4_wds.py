...
from uni_interleaved.utils.misc import get_world_size
from uni_interleaved.custom_datasets.utils.loader import BaseLoader
from uni_interleaved.custom_datasets.utils.wds_utils import (
...
from uni_interleaved.custom_datasets.utils.wds_utils import interleaved_collation_fn
...
def preprocess_mmc4_data(
    ...
    for sample_image, sim_vec in zip(info["image_info"], sim_matrix):
        ...
        image_path = osp.join(data_root, image_name[:12]+'.jpg')
        try:
    ...
    image_subseq = "<|startofimage|>" + "<|sniffer|>" * num_img_token
    ...
    text = (
        text
        .replace("<|sniffer|> ", "<|sniffer|>")
        .replace(" <|sniffer|>", "<|sniffer|>")
        .replace(" <|startofimage|>", "<|startofimage|>")
        .replace("<|startofimage|> ", "<|startofimage|>")
    )
...
def build_interleaved_dataset(
    ...
    if "docs_no_face_shard" in input_shards:
        lost_number=[3218,3267,5064,5146,7119,8991,9750,11899,15127,15252,16996,17369,17499,17818]
        shards_list=[i for i in shards_list if int(i.split('_')[-2]) not in lost_number]
    ...
    soi_token_id = tokenizer.additional_special_tokens_ids[
        tokenizer.additional_special_tokens.index("<|startofimage|>")
    ]
    image_token_id = tokenizer.additional_special_tokens_ids[
        tokenizer.additional_special_tokens.index("<|sniffer|>")
    ]
...
if __name__ == "__main__":
    from uni_interleaved.custom_datasets.utils import create_transform
    ...
    dataset = build_mmc4_webdataset(
        ...
        annt_root="./datasets/data/",
        transform=transform,
        tokenizer_path="./assets/lmsys/vicuna-7b-v1.5",
        per_device_batch_size=32,
        input_shards="docs_no_face_shard_{888..888}_v2.jsonl",
        num_samples=1000,
        resampled=False,
        floor=False,
        seed=42,
