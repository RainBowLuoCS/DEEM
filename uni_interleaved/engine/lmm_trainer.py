...
from ..utils.ref_cap_score import ref_caption_eval
from ..utils.imagenet_score import imagenet_eval
from ..custom_datasets.train.mmc4_wds import WdsDataset
from ..custom_datasets.train.mix_dataset import RandomMixWdsDataset,DatasetWrapper
...
class LMMTrainer(transformers.Trainer):
    ...
    def get_train_dataloader(self) -> DataLoader:
        ...
        if isinstance(self.train_dataset, wds.DataPipeline) or isinstance(
            self.train_dataset, RandomMixWdsDataset) or isinstance(self.train_dataset,DatasetWrapper):
        ...
        **kwargs,
        ...
        for step, inputs in enumerate(dataloader):
            ...
            if generate_mode =="generate_texts":
                ...
                gt_caption = [d[1] for d in inputs["meta"]]

                for text, sample_id, sample_caption in zip(generate_texts, data_index, gt_caption):
                    caption_result.append(
                        {"image_id": sample_id, 
                         "caption": text.strip(),
                         "gt_caption": sample_caption}
                    )
            ...
            elif generate_mode=="generate_imagenet":
                generate_texts = self.tokenizer.batch_decode(
                    outputs["text_ids"], skip_special_tokens=True
                )
                data_index = [d[0] for d in inputs["meta"]]
                answers = [d[2] for d in inputs["meta"]]
                image_paths= [d[3] for d in inputs["meta"]]
                for text, sample_id, answer, image_path in zip(generate_texts, data_index, answers, image_paths):
                    caption_result.append(
                        {"image_id": sample_id, 
                         "caption": text.strip(),
                         "gt_caption": answer,
            ...
            elif generate_mode == "generate_referring":
                generate_texts = self.tokenizer.batch_decode(
                    outputs["text_ids"], skip_special_tokens=True
                )
                data_index = [d[0] for d in inputs["meta"]]
                for text, meta in zip(generate_texts, inputs["meta"]):
                    ...
                    caption_result.append(
                        {
                            "gt_caption": caption,
                            "caption": text.replace("#"," ").replace("user:"," ").replace("asistant:"," ").strip(),
                        }
                    )
                    # print(text.replace("## user:"," ").strip())
        ...
        if generate_mode == "generate_texts":
            ...
            if self.accelerator.is_main_process:
                if hasattr(dataloader.dataset, "question_file") and dataloader.dataset.question_file is None:
                    # gqa eval use python eval.py scripts to evaluate
                    eval_func = (
                        partial(vqa_eval,question_file=dataloader.dataset.question_file, eval=False)
                    )
                    _metrics = eval_func(
    ...
    def evaluation_loop(
        ...
        if generate_mode in ("generate_texts", "generate_vqa", "generate_grounding","generate_referring","generate_imagenet"):
