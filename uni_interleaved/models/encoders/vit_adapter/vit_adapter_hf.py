...
class CLIPVisionTransformerAdapter(nn.Module):
    def __init__(self, config: CLIPVisionConfig, conv_inplane=64, n_points=4):
        ...
        elif num_attention_heads == 12: # for ViT-Base
            self.interaction_indexes = [[0, 2], [3, 5], [6, 8], [9, 11]]
            self.c1_proj=nn.Linear(embed_dim,1024)
            self.c2_proj=nn.Linear(embed_dim,1024)
            self.c3_proj=nn.Linear(embed_dim,1024)
            self.c4_proj=nn.Linear(embed_dim,1024)
            self.fc_proj=nn.Linear(embed_dim,1024)
        else:
    ...
    def forward(
        ...
        if self.config.num_hidden_layers==12:
            c1=self.c1_proj(c1.permute(0,2,3,1)).permute(0,3,1,2)
            c2=self.c2_proj(c2.permute(0,2,3,1)).permute(0,3,1,2)
            c3=self.c3_proj(c3.permute(0,2,3,1)).permute(0,3,1,2)
            c4=self.c4_proj(c4.permute(0,2,3,1)).permute(0,3,1,2)
            last_hidden_state=self.fc_proj(last_hidden_state)
            
        multiscale_features = [c1, c2, c3, c4]
        
        pooled_output = last_hidden_state[:,0:1,:]
...
def clip_vit_adapter_hf(
    model_path="assets/openai/clip-vit-base-patch16",
    image_size=224,
    freeze=False,
    freeze_vit=False,
    ...
    if "vit-base" in model_path:
        image_size=256
    elif "vit-large" in model_path:
        image_size=224
    else:
