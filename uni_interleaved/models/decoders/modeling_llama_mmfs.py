""" Modifed from PyTorch QLLaMA model."""
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        attn_weights = torch.matmul(query_states * (self.head_dim ** -0.5), key_states.transpose(2, 3))
...
def deform_inputs(hidden_states, vision_hidden_states, spatial_shapes=[(16, 16)]):
...
class LlamaMMFSAttention(nn.Module):

    def __init__(self, config: LlamaConfig, layer_idx):
        super().__init__()
        self.layer_idx = layer_idx
        self.config = config
        self.spatial_shapes = [(s, s) for s in config.spatial_shapes]
        self.hidden_size = config.hidden_size
        self.num_heads = config.num_attention_heads
        self.head_dim = self.hidden_size // self.num_heads
        self.max_position_embeddings = config.max_position_embeddings
        self.vision_hidden_size = config.image_embed_dim
        ...
        self.attn = MMFS(
            ...
            d_query=self.hidden_size,
            d_value=self.vision_hidden_size,
            d_out=self.hidden_size,
            n_levels=len(self.config.spatial_shapes),
            n_heads=16,
            n_points=8,
            ratio=self.vision_hidden_size / self.hidden_size,
            offset_init_magnitude=3.0,
    ...
    def forward(
            self,
            hidden_states: torch.Tensor,
            vision_hidden_states: Optional[Tuple[torch.Tensor]] = None,
            cross_attention_mask: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        hidden_states = self.norm1(hidden_states)
        vision_hidden_states = self.norm2(vision_hidden_states)

        reference_points, spatial_shapes, level_start_index = deform_inputs(hidden_states, vision_hidden_states, self.spatial_shapes)
...
class LlamaDecoderLayer(nn.Module):
    def __init__(self, config: LlamaConfig, use_cross_attn: bool, layer_idx):
        super().__init__()
        self.hidden_size = config.hidden_size
        self.self_attn = LlamaAttention(config=config)
        self.layer_idx = layer_idx
        self.llama_cross_attn = LlamaMMFSAttention(config=config, layer_idx=layer_idx) if use_cross_attn else None
        self.mlp = LlamaMLP(
    ...
    def forward(
        ...
        if self.llama_cross_attn is not None and vision_hidden_states is not None:
            residual = hidden_states
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def __init__(self, config: LlamaConfig):
        ...
        self.layers = nn.ModuleList(
            [LlamaDecoderLayer(config, use_cross_attn[idx], idx) for idx in range(config.num_hidden_layers)])
...
class LlamaForCausalLM(LlamaPreTrainedModel):
    ...
    def prepare_inputs_for_generation(
            ...
            vision_hidden_states=None, cross_attention_mask=None, **kwargs
    ):
        if past_key_values:
