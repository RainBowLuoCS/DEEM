...
class MMInterleaved(nn.Module):
    def __init__(
        ...
        *,
        ...
        special_token_dict: dict = dict(
            ...
            refleft_token_id=32002,
            refright_token_id=32003,
            boxleft_token_id=32004,
            boxright_token_id=32005
        ),
        ...
        self.loss_sniffer_weight = loss_sniffer_weight
        ...
        if image_decoder_config is not None:
            ...
            decoder_config=dict(uncond_prob=image_decoder_config.pop("uncond_prob",0.1),
                            seq_len=image_decoder_config.pop("seq_len", 77),
                            embed_dim=image_decoder_config.pop("embed_dim", 1024),
                            perceiver_config=image_decoder_config.pop("perceiver_config",None))
            ...
            self.visual_tokenizer = VisualTokenizer(
                ...
                **image_decoder_config,
                ...
                diffusion_hidden_size=image_embed_dim,
                sd_use_encoder=True,
                freeze_vfm=freeze_vfm,
                freeze_dm=freeze_dm,
        ...
        llm_model.requires_grad_(not freeze_llm)
        ...
        if image_decoder_config is not None:
            self.image_decoder = ImageDecoder(
                **decoder_config,
                decoder=self.visual_tokenizer.encoder,
            )
            self.visual_tokenizer.neg_prompt_embeds=self.image_decoder.neg_prompt_embeds
        else:
    ...
    def _prepare_mm_embeds(
        ...
        image_tensors_mask: Optional[torch.FloatTensor] = None,
        num_image_per_seq: Optional[torch.Tensor] = None,
        meta: Optional[Union[torch.Tensor, List]] = None,
        option_inference: Optional[bool] = False
    ):
        output = {}
        ...
        if option_inference:
            visual_output = self.visual_tokenizer(image_tensors[0:1], image_tensors_dec[0:1], image_tensors_mask[0:1])
            visual_output["vis_embed"]=torch.repeat_interleave(visual_output["vis_embed"],image_tensors.shape[0],0)
            visual_output["multiscale_features"]=[torch.repeat_interleave(i,image_tensors.shape[0],0) for i in visual_output["multiscale_features"]]
        else:
    ...
    def forward(
        ...
        **kwargs,
        ...
        loss = loss + loss_sniffer.mean() * loss_sniffer_weight
    ...
    def generate_images(
        ...
        **kwargs,
        ...
        # target_image_idxs=torch.tensor([i+2 for i in range(0,30,3)],device=context_features.device)
        # print(mmfs_mask)
    ...
    def generate_scores(
        ...
        **kwargs,
        ...
        for i in range(len(text_ids)):
            ...
            _image_tensors_dec = image_tensors_dec[[i]].expand(
                options_ids[i].shape[0], -1, -1, -1
            )
            _image_tensors_mask = image_tensors_mask[[i]].expand(
                options_ids[i].shape[0], -1, -1, -1
            )
            ...
            for j in range(mini_iter):
                ...
                _output = self._prepare_mm_embeds(
                    ...
                    image_tensors_dec=_image_tensors_dec[j * mini_bs : (j + 1) * mini_bs],
                    image_tensors_mask=_image_tensors_mask[j * mini_bs : (j + 1) * mini_bs],
                    num_image_per_seq=_num_image_per_seq[
                        j * mini_bs : (j + 1) * mini_bs
                    ],
                    meta=None,
                    option_inference=True
                )
                mm_embeds = _output.pop("mm_embeds")
                cross_attention_mask = _output.pop("cross_attention_mask")
                mmfs_features_mm = _output.pop("mmfs_features_mm")
    ...
    def generate(
        ...
        **kwargs,
        ...
        elif mode in ("generate_texts", "generate_vqa", "generate_grounding","generate_referring","generate_imagenet"):
