...
# new_image_root="./datasets/mmc4/mmc4_images_v2"

def move_file(json_file_name,new_images_root):
    data=json.load(open(json_file_name,'r'))
    old_name=os.path.join("/".join(json_file_name.split('/')[:-1]),f"{data['key']}.jpg")
    new_name=os.path.join(new_images_root,f"{data['local_identifier'][:12]}.jpg")
...
def image_rename_script(args):
    ...
    file_names=[]
    for i in dir_names:
        file_names.extend([os.path.join(i,j) for j in os.listdir(i) if j.endswith(".json")])
...
if __name__ == '__main__':
    def parse_args():
        ...
        parser.add_argument(
            "--images_root", default="/home/luorun/workspace/NIPS/mmc4_images")
        parser.add_argument(
            "--new_images_root", default="/home/luorun/workspace/NIPS/mmc4_images_v2")
