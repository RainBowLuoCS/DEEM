...
import fsspec
from img2dataset.logger import CappedCounter,write_stats
...
def _download_image(row, timeout, user_agent_token, disallowed_header_directives,sample_writer,compute_key):
    """Download an image with urllib"""
    key, url = row
    img_stream = None
    file_name=f"{sample_writer.subfolder}/{compute_key(key)}.{sample_writer.encode_format}"
...
def _download_image_with_retry(row, timeout, retries, user_agent_token, disallowed_header_directives,sample_writer,compute_key):
    for _ in range(retries + 1):
        key, img_stream, err = _download_image(row, timeout, user_agent_token, disallowed_header_directives,sample_writer,compute_key)
        if img_stream is not None:
            return key, img_stream, err
    return key, None, err


def _compute_key(shard_id, oom_sample_per_shard, oom_shard_count, key):
...
def _download_shard(
    ...
    compute_key=partial(_compute_key,shard_id, oom_sample_per_shard, self.oom_shard_count)

    with ThreadPool(self.thread_count) as thread_pool:
        for key, img_stream, error_message in thread_pool.imap_unordered(
            lambda x: _download_image_with_retry(
                x,
...
def replace_img2dataset():
    Downloader.download_shard=_download_shard